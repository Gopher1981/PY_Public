...
def startup():
 global storeNumber
 storeNumber = str(input("\"q\" to exit \nInput store number: "))
 if storeNumber == "q" or storeNumber =="Q":
  exit()

def storeLengthCorrect():
 global storeNumber
 global fullStoreNumber
 fullStoreNumber = storeNumber.zfill(5)
 print("\nAdjusted store number is now " + fullStoreNumber + "\nOriginal store number is still " + storeNumber)

def storeLengthCheck():
 global storeNumber
 global fullStoreNumber
 if not storeNumber.isdigit():
  print("That is not a number, please only use 0-9")
  retryLoop()
 elif len(storeNumber) > 5:
  print("\nYour store number is too long, should only be 5 digits or less")
 else:
     if not 'storeNumber' in globals() or len(storeNumber) == 0:
      print("\nStore number is not set")
     else:
      print("\nStore number is set to " + storeNumber)
     if len(storeNumber) == 5:
      print("Store number is 5 digits long")
      fullStoreNumber = storeNumber
     elif len(storeNumber) != 5:
      storeReport = "\nStore number should be 5 digits long \nbut is only {} digits long"
      print(storeReport.format(len(storeNumber)))
      storeLengthCorrect()
